...
const makeFreshDefaults = (): SimpleEventFormValues => {
  const now = Date.now();
...
export function SimpleCreatorForm({ onSubmit, steps }: SimpleCreatorFormProps) {
  ...
  const form = useSimpleEventForm(makeFreshDefaults());
  ...
  useEffect(() => {
    ...
    if (draft) {
      ...
      setDraftData({
        ...
        values: draft.values as SimpleEventFormValues,
  ...
  const submit = handleSubmit(
    useCallback(
      async (values) => {
        try {
          const resultEventId = await onSubmit(
            values as SimpleEventFormValues,
            coverImageFile
          );

          // If we have a cover image, upload it
          if (resultEventId && coverImageFile) {
            try {
              await uploadEventCover(resultEventId, coverImageFile, {
                onStart: () => setIsCoverUploading(true),
                onSuccess: () => {
                  queryClient.invalidateQueries({
                    queryKey: ['GetEventDetail', resultEventId],
                  });
                  queryClient.invalidateQueries({
          ...
          console.error('[Submit] Failed to create event:', error);
        }
      },

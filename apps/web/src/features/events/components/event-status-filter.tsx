...
interface StatusOption {
  ...
  labelKey: keyof typeof import('@/lib/i18n/locales/en').en.myEvents.filters;
...
export interface EventStatusFilterProps {
  values: EventStatusFilterValue[];
  onChange: (values: EventStatusFilterValue[]) => void;
}
  ...
  const options = useMemo(
    () =>
      STATUS_OPTIONS.map((opt) => ({
        ...opt,
        label: t.myEvents.filters[opt.labelKey],
      })),
    [t]
  );
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-xs text-zinc-500 dark:text-zinc-400">
        {t.myEvents.multipleSelectionAllowed}
      </p>
    </div>
  );
}

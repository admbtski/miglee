 ...
 * Uses useDeleteEventMutation under the hood.
 */
export function DeleteEventModals({
  deleteId,
  onClose,
  onSuccess,
  ...
  successSubtitle = 'The event has been permanently deleted.',
  errorTitle = 'Event not deleted',
  errorSubtitle = 'We could not delete this event. Please try again later.',
}: Props) {
  const { mutateAsync: deleteEventMutateAsync } = useDeleteEventMutation();
  ...
  const handlePrimary = useCallback(async () => {
    ...
    try {
      setLoading(true);
      await deleteEventMutateAsync({ id: deleteId });
  ...
  }, [deleteId, deleteEventMutateAsync, onClose, onSuccess]);

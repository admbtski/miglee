...
export function computeJoinState(
  now: Date,
  event: EventJoinConfig
): JoinStateResult {
  // 1) Okno otwarcia zapisów (przed startem)
  const opensAt =
    event.joinOpensMinutesBeforeStart != null
      ? new Date(
          event.startAt.getTime() - event.joinOpensMinutesBeforeStart * 60_000
        )
      : null;
  ...
  const preCutoffAt =
    event.joinCutoffMinutesBeforeStart != null
      ? new Date(
          event.startAt.getTime() - event.joinCutoffMinutesBeforeStart * 60_000
        )
      : null;
  const isPreCutoffClosed = preCutoffAt
    ? now >= preCutoffAt && now < event.startAt
    : false;
  ...
  if (now >= event.startAt && now < event.endAt) {
    if (event.allowJoinLate) {
      if (event.lateJoinCutoffMinutesAfterStart != null) {
        const lateCutoffAt = new Date(
          event.startAt.getTime() +
            event.lateJoinCutoffMinutesAfterStart * 60_000
        );
  ...
  // 4) Ręczne zamknięcie
  const isManuallyClosed = !!event.joinManuallyClosed;

  // 5) Pojemność
  const isFull = event.joinedCount >= event.max;
  ...
  if (event.joinMode === 'REQUEST') ctaLabel = 'Wyślij prośbę';
  if (event.joinMode === 'INVITE_ONLY') ctaLabel = 'Tylko zaproszenie';
  ...
  if (isFull) {
    ...
    if (event.joinMode === 'OPEN' && windowOpen) {
      canJoin = true;
      ctaLabel = 'Dołącz';

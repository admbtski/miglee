...
import { EditLinkModal } from '../../../app/[locale]/event/[id]/manage/invite-links/_components/edit-link-modal';
import {
  PlanUpgradeBanner,
...
export function InviteLinksPanel({ eventId }: InviteLinksPanelProps) {
  ...
  const { data, isLoading, refetch } = useEventInviteLinksQuery({
    eventId,
  ...
  const createLink = useCreateEventInviteLinkMutation({
    onSuccess: () => {
      refetch();
    },
  });

  const revokeLink = useRevokeEventInviteLinkMutation({
    onSuccess: () => {
      refetch();
    },
  });

  const deleteLink = useDeleteEventInviteLinkMutation({
    onSuccess: () => {
      refetch();
    },
  });

  const links = data?.eventInviteLinks ?? [];

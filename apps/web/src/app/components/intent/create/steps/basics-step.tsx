...
import { CategoryMultiCombo } from '../components/category-combobox';

export function BasicsStep({
  form,
  interests, // [{id,name}] – można podać jako initialOptions
}: {
  ...
  // map selected ids -> full objects for the multi-combo
  const interestIds = useWatch({ control, name: 'interestIds' }) as string[];

  const selectedObjects: CategoryOption[] = useMemo(() => {
    const byId = new Map(interests.map((i) => [i.id, i.name]));
  ...
  // keep capacity in sync with mode (opcjonalnie)
  useEffect(() => {
  ...
  return (
    <div className="space-y-8">
      ...
      <Controller
        ...
        control={control}
        render={({ field }) => {
          const onChangeMulti = (vals: CategoryOption[]) => {
            const ids = vals.map((v) => v.id);
            field.onChange(ids);
            // natychmiastowa walidacja granic 1–3
          ...
          return (
            <div className="group">
              ...
              <div
                ...
                className="mt-1 text-xs text-red-500"
              >
                {errors.interestIds?.message as string}
              </div>
            </div>
          );
        }}

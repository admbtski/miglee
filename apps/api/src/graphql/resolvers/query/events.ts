...
function getAndArray(where: Prisma.EventWhereInput): Prisma.EventWhereInput[] {
  if (!where.AND) where.AND = [];
  return where.AND as Prisma.EventWhereInput[];
}

function buildBaseWhere(args: QueryEventsArgs): Prisma.EventWhereInput {
  ...
  const shouldHideCanceledAndDeleted =
    !status ||
    (status !== EventStatus.Canceled && status !== EventStatus.Deleted);
  ...
  /* ───────────────────────── Verified Owner filter ─────────────────────────
     Matches events where OWNER's user.verifiedAt is not null.
  -------------------------------------------------------------------------- */
...
function buildOrderBy(args: QueryEventsArgs) {
  ...
  switch (args.sortBy) {
    case EventsSortBy.StartAt:
      return [
        { boostedAt: { sort: 'desc', nulls: 'last' } },
        { startAt: dir },
        { id: 'desc' },
      ] as Prisma.EventOrderByWithRelationInput[];
    case EventsSortBy.CreatedAt:
      return [
        { boostedAt: { sort: 'desc', nulls: 'last' } },
        { createdAt: dir },
        { id: 'desc' },
      ] as Prisma.EventOrderByWithRelationInput[];
    case EventsSortBy.UpdatedAt:
      return [
        { boostedAt: { sort: 'desc', nulls: 'last' } },
        { updatedAt: dir },
        { id: 'desc' },
      ] as Prisma.EventOrderByWithRelationInput[];
    case EventsSortBy.MembersCount:
      return [
        { boostedAt: { sort: 'desc', nulls: 'last' } },
        { members: { _count: dir } },
        { startAt: 'asc' },
...
function applyStatusWhere(
  ...
  if (!status || status === EventStatus.Any) return;
  ...
  switch (status) {
    ...
    case EventStatus.Canceled:
      ...
      const andArray = where.AND as Prisma.EventWhereInput[];
    ...
    case EventStatus.Deleted:
      ...
      const andArray2 = where.AND as Prisma.EventWhereInput[];
...
>(args: QueryEventsArgs, now: Date): (a: T, b: T) => number {
...
export const eventQuery: QueryResolvers['event'] = resolverWithMetrics(
  'Query',
  'event',
  async (_p, { id }, { user }) => {
